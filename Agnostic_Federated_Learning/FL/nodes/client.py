...
class CreateDataset(Dataset):
    ...
    def __init__(self, dataset, idxs, noise_level):
...
class LocalBase():
    def  __init__(self,args,train_dataset,test_dataset,client_id):
        ...
        self.trainDataset=CreateDataset(train_dataset, args.train_distributed_data[client_id], noise_level=noise_level)
        self.testDataset=CreateDataset(test_dataset, args.test_distributed_data[client_id], noise_level=noise_level)
    ...
    def add_gaussian_noise(self, data, variance):
        # Assuming data is a tensor
        noise = torch.randn_like(data) * variance
        return data + noise
        ...
        with torch.no_grad():
            for images, labels in self.testDataloader:
                ...
                for c in range(num_classes):
                    class_mask = labels == c
                    class_correct[c] += pred[class_mask].eq(labels[class_mask].view_as(pred[class_mask])).sum().item()
        ...
        print('| Client id:{} | Test_Loss: {:.3f} | Test_Acc: {:.3f}'.format(self.client_id,test_loss, test_acc))
       	...
       	for c in range(num_classes):
            if (class_total[c] != 0):
                ...
                print('| Class {}: {:.3f}%'.format(c, class_acc))
